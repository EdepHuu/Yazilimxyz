...
type MerchantSelf = {
  id: number; companyName: string; iban: string; taxNumber: string;
  companyAddress: string; phone: string;
};

type ProductListItem = {
  id: number; name: string; isActive: boolean;
  categoryName?: string | null; price?: number | null; basePrice?: number | null;
};
...
type ProductRow = {
  id: number; name: string; isActive: boolean;
  categoryName?: string | null; price?: number | null;
  stockTotal: number; imageUrl?: string | null;
};
...
export default function MerchantProductsListPage() {
  ...
  const [q, setQ] = useState(''); const [page, setPage] = useState(1); const [pageSize, setPageSize] = useState(10);
    ...
    (async () => {
      try {
        ...
        const enriched: ProductRow[] = await Promise.all(
          base.map(async (p): Promise<ProductRow> => {
            let stockTotal = 0; let imageUrl: string | null = null;
            ...
            return { id: p.id, name: p.name, isActive: !!p.isActive, categoryName, price, stockTotal, imageUrl };
          })
        );
        ...
        if (axios.isAxiosError(err)) {
          ...
          setMsg((err.response?.data as { message?: string })?.message || `Hata: ${code ?? ''}`);
        } else setMsg('Beklenmeyen bir hata oluştu.');
  ...
  const filtered = useMemo(() => {
    ...
    const base = term
      ? rows.filter(r => (r.name||'').toLowerCase().includes(term) || (r.categoryName||'').toLowerCase().includes(term))
      : rows;
    ...
    <div className="max-w-6xl">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5">
            ...
            <tbody>
              {filtered.items.map((p) => (
                <tr key={p.id} className="border-t border-slate-100">
                  ...
                  <td className="py-3 px-3 text-slate-700">{p.price != null ? `${p.price.toLocaleString('tr-TR')} ₺` : '-'}</td>
                  <td className="py-3 px-3">{p.stockTotal}</td>
                  <td className="py-3 px-3">
                    <span className={'inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold ' + (p.isActive ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-600')}>
              ...
              {filtered.items.length === 0 && (
                <tr><td colSpan={6} className="py-10 text-center text-slate-500">Kayıt bulunamadı.</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="flex items-center justify-between mt-4 text-sm text-slate-600">
          <div>Toplam <strong>{q ? filtered.total : apiTotal}</strong> ürün</div>
          <div className="flex items-center gap-2">
            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={filtered.page <= 1} className="px-3 py-1.5 rounded-lg border border-slate-300 disabled:opacity-50">‹ Önceki</button>
            <span>{filtered.page} / {filtered.maxPage}</span>
            <button onClick={() => setPage(p => Math.min(filtered.maxPage, p + 1))} disabled={filtered.page >= filtered.maxPage} className="px-3 py-1.5 rounded-lg border border-slate-300 disabled:opacity-50">Sonraki ›</button>
          </div>
        </div>
      </div>
    </div>
